...
%
Alive2 supports 16 (\texttt{half}), 32 (\texttt{float}), 64-bit
(\texttt{double}) and 128 (\texttt{fp128}) bits floating point types.
%
Non-IEEE-754 compliant floating point types are not supported, such as
the x86 80-bit floating type as it is not supported by z3's FPA
theory.
%
Also not supported are \frem instructions, which are not supported by
...
\item NNaN allows optimizations to assume the arguments and result are
not NaN. If an argument is a NaN, or the result would be a NaN, it
produces a poison value instead.
%
\item NInf flag allows optimizations to assume the arguments and
result are not positive or negative infinities. If an argument is
positive or negative infinities, or the result would be positive or
negative infinities, it produces a poison value instead.
%
\item No Signed Zeros - Allow optimizations to treat the sign of a
zero argument or zero result as insignificant. This does not imply
