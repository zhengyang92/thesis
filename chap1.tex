...
Performance is vital in computer software. Faster software can lead to
better user experiences, more efficient resource utilization, and
lower costs. As a result, developers and researchers are constantly
looking for ways to optimize software to run faster and more
efficiently.

However, optimizing software is a challenging task, especially in the
era where Moore's Law is no longer valid: the performance gains from
improvements in single core performance are diminishing, and the
complexity of software is increasing.
...
One approach to addressing these challenge is human experts manually
optimizing code. However, this process is time-consuming and
