...
synthesizing high-performance collective communication algorithms
that are proven to be correct and optimal for a given network topology, and
a synthesizing superoptimizer that can automatically generate peephole
optimizations for vector instructions that are missing from a commodity
compiler.
...
rewrite the code from scratch in low level languages, such as assembly
and C with intrinsics, to take advantage of the unique features of the
architecture.
%
However, because of the increasing complexity of modern hardware, this
process is time-consuming and error-prone.
%
On the other hand, the complexity of modern software makes it even
...
%
Moreover, programs optimized in this way are likely not portable and
